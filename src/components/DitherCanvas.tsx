import { useRef, useEffect, useImperativeHandle, useCallback, type Ref, useMemo } from 'react';
...
export interface DitherCanvasRef {
  ...
  renderFrame: (directOptions?: DitherOptions, directImage?: HTMLImageElement) => Promise<void>;
};

export const DitherCanvas = ({ image, options, onProcessComplete, ref }: {
  ...
  const renderFrame = useCallback(async (directOptions?: DitherOptions, directImage?: HTMLImageElement) => {
    ...
    if (offscreenSupported && offscreenCanvasRef.current && canvasRef.current) {
      success = await webglServiceRef.current.ditherImage(localImage, directOptions || options);
      ...
      if (success) {
        ...
        if (visibleContext) {
          ...
          offscreenCanvasRef.current.convertToBlob()
            ...
            .then(imageBitmap => {
              ...
              visibleContext.clearRect(0, 0, canvasRef.current?.width as number, canvasRef.current?.height as number);
              visibleContext.drawImage(imageBitmap, 0, 0, imageBitmap.width, imageBitmap.height, 0, 0, localImage.width, localImage.height);
        ...
        if (currentCanvas && canvasRef.current) {
          ...
          if (tempCtx) {
            ...
            if (ctx) {
              ...
              ctx.drawImage(tempCanvas, 0, 0, tempCanvas.width, tempCanvas.height, 0, 0, localImage.width, localImage.height);
            }
          }
        }
      }
  ...
  useEffect(() => {
    ...
    if (offscreenSupported) {
      ...
      console.log('WebGL setup on OffscreenCanvas:', success ? 'success' : 'failed');
    } else if (canvasRef.current) {
