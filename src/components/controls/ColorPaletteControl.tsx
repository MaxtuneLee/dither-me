...
export function ColorPaletteControl({ options, onOptionsChange }: ColorPaletteControlProps) {
    // Track custom colors locally to avoid unnecessary re-renders
    const [customColors, setCustomColors] = useState<string[]>(options.customPalette || ['#000000', '#FFFFFF']);
    ...
    const handlePaletteToggle = useCallback((enabled: boolean) => {
        onOptionsChange({
            ...options,
            usePalette: enabled
        });
    ...
    const handlePaletteTypeChange = useCallback((value: string) => {
        const paletteType = Number.parseInt(value);
        onOptionsChange({
            ...options,
            paletteType,
            // Reset user palette selection when switching types
            selectedUserPalette: paletteType === PaletteType.USER ? userPalettes[0]?.id : undefined
        });
    }, [options, onOptionsChange, userPalettes]);

    const handlePaletteColorsChange = useCallback((value: string) => {
    ...
    const handleColorBlockClick = useCallback((index: number, event: React.MouseEvent) => {
    ...
    const handleColorPickerChange = useCallback((color: ColorResult) => {
    ...
    const handleUserPaletteChange = useCallback((value: string) => {
    ...
    const handleClickOutside = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (!target.closest('.color-picker-container') && !target.closest('.color-block')) {
                ...
                <div onClick={handleClickOutside}>
                    ...
                    {options.paletteType === PaletteType.USER && (
                        <div className="mb-4">
                            ...
                            <Select
                                value={String(options.selectedUserPalette ?? userPalettes[0]?.id)}
                            ...
                            <div className="mt-2 flex gap-1">
                                {userPalettes.find(p => p.id === options.selectedUserPalette)?.colors.map((color) => (
                                    <div
                                        key={color}
                    ...
                    {options.paletteType !== PaletteType.CUSTOM && options.paletteType !== PaletteType.USER && (
                        <div className="mb-4">
