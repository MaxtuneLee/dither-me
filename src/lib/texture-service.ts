 ...
 */
export function createMainTexture(gl: WebGLRenderingContext): WebGLTexture | null {
 ...
 * @returns The created texture object or null if creation failed
 */
export function createErrorTexture(gl: WebGLRenderingContext): WebGLTexture | null {
  const texture = gl.createTexture();
  if (!texture) return null;
